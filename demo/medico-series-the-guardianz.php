...
$product=$objMain->getRow("select * from product where id=16");
...
if(!empty($product_size)){ $i=0;
    foreach($product_size as $psize){ $i++; if($i==1) { $first_size=$psize['dimension']; $checked='image-checkbox-checked'; $title='T.T'; } else { $checked=''; $title='E.T'; } ?>
<div style="float: left; position: relative;  margin:5px; text-align: center;" >
    <label class="image-checkbox <?php echo $checked; ?>" style="padding: 10px 30px 0px 30px; margin-top: -10px;">
      ...
      <p style="font-size: 15px; "><?php echo $title; ?> <?php echo $psize['dimension']; ?>"</p>
