...
if (typeof custom_length == 'undefined') {
  custom_length="";
}        
if (typeof custom_width == 'undefined') {
  custom_width="";
}
...
if(error==0){
 ...
 <script type="text/javascript">
    function addtocart2(productid){ var error=0;
        var dimension=$('input[name=size]:checked').val();
        var product_color=$('input[name=color]:checked').val();
        var product_size=$("#product_size").val();
...
<script type="text/javascript">
    function updatecart(productid, key){
        var price=$("#product_price_"+key).val();
        var qty=$("#qty"+key).val();
                var product_color=$('input[name=color'+key+']:checked').val();
  ...
  product_color='';
        ...
        $.ajax({
            url: 'lib/ajax.php',
            type: 'post',
            data: { 'type' : 'updatecart', 'productid' : productid, 'qty' : qty, 'prodkey' : key,'product_color' :product_color},
                beforeSend: function(){
                },
            success: function(obj) { 
                    data = jQuery.parseJSON(obj);
     ...
     function updatefreeproductcolor(productid, key){
        var price=$("#product_price_"+key).val();
                var product_color=$('input[name=freecolor'+key+']:checked').val();
  ...
  product_color='';
        ...
        $.ajax({
            url: 'lib/ajax.php',
            type: 'post',
            data: { 'type' : 'updatefreeproductcolor', 'productid' : productid, 'prodkey' : key,'product_color' :product_color},
                beforeSend: function(){
                },
            success: function(obj) { 
                    data = jQuery.parseJSON(obj);
...
<script type="text/javascript">
 function couponvalidate2(){
     var coupon='';
      <?php 
     if(isset($_SESSION['dic_coupon_code']) && !empty($_SESSION['dic_coupon_code']))
                            {?>
         coupon='<?php echo $_SESSION['dic_coupon_code']; ?>';          
...
if(!empty($product_size)){ $i=0;
    foreach($product_size as $psize){ 
        $mrp_price='mrp_price['.$psize['dimension'].']=[0';
        $product_price='price['.$psize['dimension'].']=[0'; 
        $prices=$objMain->getResults("select * from product_price where product_id=".$product_id." and dimension=".$psize['dimension']);
if(!empty($prices)){ $i=0;
    foreach($prices as $price){ 
                $dis_price=$price['product_price']*(10/100);
        $mrp_price=$mrp_price.',"'.$objMain->INR($price['product_price']+$dis_price).'"'; 
        $product_price=$product_price.',"'.$objMain->INR($price['product_price']).'"'; }
}
$product_price.='];'; 
$mrp_price.='];'; 
echo $product_price;
echo $mrp_price;
    ...
    ?>  
    var act_price=parseInt(price[size][size_value].replace(/,/g, ''));

     <?php if(isset($_SESSION['dic_coupon_code']) && !empty($_SESSION['dic_coupon_code']) && ($product_id==29 || $product_id==30 || $product_id==31))
                            {
        if(isset($_SESSION['dic_coupon_type']) && $_SESSION['dic_coupon_type']=='percentage')
                            {
                                ?>
         var dis_value='<?php echo $_SESSION['dic_coupon_value']; ?>';

         var dis_price= Math.round(act_price*(dis_value/100)); 
         act_price= act_price-dis_price;      

            <?php } else{?>
                 var dis_value='<?php echo $_SESSION['dic_coupon_value']; ?>';
            act_price= act_price-dis_value;      
            <?php }} ?>

    $("#discount_price").html(INR(act_price));
    $("#mrp_price").html(mrp_price[size][size_value]);
...
function customprice(){
    ...
    <?php 
 if($product_id==29 || $product_id==30 || $product_id==31|| $product_id==34) {
...
echo $product_price;
    ...
    if(error==0 && length!="" && width!=""){
        ...
        <?php if($product_id==29 || $product_id==30 || $product_id==31|| $product_id==34) { ?>
        ...
        var price=pricev;
         var act_price=parseInt(price.replace(/,/g, ''));;
         var act_price2=parseInt(price.replace(/,/g, ''));;
     ...
     <?php if(isset($_SESSION['dic_coupon_code']) && !empty($_SESSION['dic_coupon_code']) && ($product_id==29 || $product_id==30 || $product_id==31))
        ...
        if(isset($_SESSION['dic_coupon_type']) && $_SESSION['dic_coupon_type']=='percentage')
         ...
         act_price= act_price-dis_price;      
            ...
            <?php } else{?>
                 var dis_value='<?php echo (isset($_SESSION['dic_coupon_value']))? $_SESSION['dic_coupon_value']:''?>';
            act_price= act_price-dis_value;      
            <?php }} ?>
            var mrp_dis=Math.round(act_price2*(10/100))+act_price2;

        $("#discount_price").html(INR(act_price));
        $("#mrp_price").html(INR(mrp_dis));
...
<script type="text/javascript">
function productfilter(size,productid){ 
            var dimension=$('input[name=size]:checked').val();
        ...
        $.ajax({
            url: 'lib/ajax.php',
            type: 'post',
            data: { 'type' : 'productfilter', 'size' : size, 'dimension' : dimension, 'productid' : productid},
                beforeSend: function(){
                },
