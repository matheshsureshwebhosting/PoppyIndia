...
//if(isset($_REQUEST['razorpay_order_id']) && $_REQUEST['razorpay_order_id']==$_SESSION['order_token']) {
...
//echo "insert into orders_items set order_id='".$id."', product_id='".$prod['id']."', cover_image='".$prod['cover_image']."', category='".$cat_name."', product_name='".$prod['product_name']."', product_size='', price='', qty='".$free['qty']*$item['qty']."', amount='0', product_type='free', custom_length='', custom_width='', product_height=''<br>";
        ...
        */
        $objMain->Query("update orders set status='verification-process' where id=".$id);
    	...
    	$msg='Hey! A new order placed in Poppy Website. Tracking ID : '.$order['tracking_id'].', Check the link below to track the order https://bit.ly/2ZVPezL ';
    //	$objAdmin->sendsms('9642911441',$msg);
