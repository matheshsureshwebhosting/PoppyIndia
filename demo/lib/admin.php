...
class Admin extends Main
{
	...
	public function checkout(){ extract($_REQUEST);
		...
		$this->Query("insert into orders set userid='".$userid."', tracking_id='', status='initiated', name='".$name."', mobileno='".$mobileno."', emailid='".$emailid."', mobileno_alt='".$mobileno_alt."', address='".$address."', city='".$city."', state='".$state."', pincode='".$pincode."', order_notes='".$order_notes."', order_date='".$order_date."', sub_total=0, coupon_code='', coupon_value=0, net_amount=0, cancel_reason='', razorpay_order_id=''");
...
if($userid==9 || $userid==18 || $userid==88) $amount=1;
...
//echo "insert into orders_items set order_id='".$id."', product_id='".$item['productid']."', cover_image='".$product['cover_image']."', category='".$category_name."', product_name='".$product['product_name']."', product_size='".$prod_size."', price='".$prodcut_price."', qty='".$item['qty']."', amount='".($item['qty']*$prodcut_price)."', product_type='".$item['product_type']."', custom_length='".$item['custom_length']."', custom_width='".$item['custom_width']."', product_height='".$item['dimension']."'<br<br>";
            ...
            $this->Query("insert into orders_items set order_id='".$id."', product_id='".$item['productid']."', cover_image='".$product['cover_image']."', category='".$category_name."', product_name='".$product['product_name']."', product_size='".$prod_size."', price='".$prodcut_price."', qty='".$item['qty']."', amount='".($item['qty']*$prodcut_price)."', product_type='".$item['product_type']."', product_color='".$item['product_color']."', custom_length='".$custom_length_1."', custom_width='".$custom_width_1."', product_height='".$item['dimension']."'");
            // echo "select * from product_free where parent_id=".$product['id'];
            // exit;
...
//echo "insert into orders_items set order_id='".$id."', product_id='".$prod['id']."', cover_image='".$prod['cover_image']."', category='".$cat_name."', product_name='".$prod['product_name']."', product_size='', price='', qty='".$free['qty']*$item['qty']."', amount='0', product_type='free', custom_length='', custom_width='', product_height=''<br>";
                    $this->Query("insert into orders_items set order_id='".$id."', product_id='".$prod['id']."', cover_image='".$prod['cover_image']."', category='".$cat_name."', product_name='".$prod['product_name']."', product_size='', price='', qty='".$free['qty']*$item['qty']."', amount='0', product_type='free', custom_length='0', custom_width='0', product_height=''".$free_color);
                }
            }
	 ...
	 public function trialregister() { extract($_REQUEST);

	 	if(!isset($_FILES['invoice']) || $mobileno=="" || $customer_date_of_birth=="" || $invoice_no=="")
		{
			$this->redirectUrl("trial.php?msg=fillinput");
			exit;
		}
		$result = $this->getRow("SELECT * FROM orders WHERE (mobileno='".$mobileno."' or mobileno_alt='".$mobileno."')");
			...
			if (isset($_FILES['invoice']) && $_FILES['invoice']['error'] === UPLOAD_ERR_OK)
					  {
					    ...
					    $allowedfileExtensions = array('jpg', 'pdf', 'doc');
					    ...
					    if (in_array($fileExtension, $allowedfileExtensions))
					    {
					      ...
					      {
					      	 $this->Query("insert into warranty set invoice='".$newFileName."', date_of_birth='".$customer_date_of_birth."', mobileno='".$mobileno."', customer_name='".$result['name']."', invoice_no='".$invoice_no."', apply_date='".date('Y-m-d h:i:s')."'");
					       $this->redirectUrl("trial.php?msg=success"); 
					      }
					      else
					      {
					         $this->redirectUrl("trial.php?msg=notwrite"); 
					      }
					    }
					    else
					    {
					      $this->redirectUrl("trial.php?msg=exfile"); 
					    }
					  }
					  else
					  {

					     $this->redirectUrl("trial.php?msg=notup"); 
					  }
			
		} else {

		$this->redirectUrl("trial.php?msg=failed");
	} }
...
print_r($res); */
	...
	public function testemail($emailid){
...
try {
    ...
    $mail->Username   = 'support@poppyindia.com';                     //SMTP username
    $mail->Password   = 'Support@1144';                               //SMTP password
    $mail->SMTPSecure = 'tls';         //Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
    $mail->Port       = 587;                                    //TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

    //Recipients
    $mail->setFrom('support@poppyindia.com', 'Mailer');
    $mail->addAddress('techaveo@gmail.com', 'Joe User');    
    $mail->addReplyTo('support@poppyindia.com', 'Information');
