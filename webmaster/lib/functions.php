...
{	
    public function checkLogin() { extract($_REQUEST);
		...
		$res=$this->getRow("select * from pu_admin where username='".$username."' and userpassword='".md5($userpassword)."'");
		if(!empty($res)){
 ...
 {
	 $old_pass=$this->clean('old_pass');
	 $user_pass=$this->clean('user_pass');

  	 $count=$this->getResultscount("select * from pu_customers where id='".$_SESSION['userid']."' and userpassword='".$old_pass."'");
	 if($count>0) {
		 $this->Query("update pu_customers set userpassword='".$user_pass."' where id='".$_SESSION['userid']."'");
...
public function findleg($id){
	$_SESSION['parent_id']="";
		$users=$this->getResults("select * from pu_customers where parent_id in (".$id.")");
		$userid='100'; 
		if(!empty($users)){
				foreach($users as $user){
					$cnt=$this->getResultsCount("select * from pu_customers where parent_id=".$user['id']);
					if($cnt<2) { $_SESSION['parent_id']=$user['id']; return; }
					$userid.=",".$user['id'];
				}
			}
		$this->findleg($userid);
