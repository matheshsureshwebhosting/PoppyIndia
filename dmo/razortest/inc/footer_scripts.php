...
if(size_type=='custom'){ 
    ...
    if(custom_length==""){
        ...
        if(!(custom_length>=68 && custom_length<=84)) { error=1; $("#length_error").show(); }
    } 
    if(custom_width==""){
        $("#width_error").show();
...
function customprice(){
    ...
    ?> 
     ...
     var price=[]; 
    <?php $product_size=$objMain->getResults("select * from product_dimension where product_id='".$product_id."'");
    ...
    var width=$("#width").val();
    if(length!=""){
        if(!(length>=68 && length<=84)) { error=1; $("#length_error").show(); }
    } 
    if(width!=""){
